...
@MainActor
final class SwimSetViewModel: ObservableObject {
    ...
    func updateActivitySet(workoutId: String, activity: DBActivity, set: DBSwimSet) async throws {
        try await workoutDataService.updateActivitySet(workoutId: workoutId, activity: activity, set: set)
    }
    
    func getSetMetrics(workoutId: String, activity: DBActivity, setId: String) async throws -> DBSwimSet {
        let snapshot = try await workoutDataService.getActivitySet(workoutId: workoutId, activity: activity, setId: setId) as! DBSwimSet
        return DBSwimSet(
            id: setId,
